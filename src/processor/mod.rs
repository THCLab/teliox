...
use crate::{
    ...
    seal::{parse, AttachedSourceSeal, EventSourceSeal},
    state::{vc_state::TelState, ManagerTelState, State},
    tel::{event_generator, Tel},
};
...
pub fn test_processing() -> Result<(), Error> {
    ...
    // let mana_ev = processor.get_management_events(&management_tel_prefix)?;
    ...
    let iss_event =
        event_generator::make_issuance_event(&st, message_id.clone(), &derivation, &serialization)?;
    ...
    let vrt =
        event_generator::make_rotation_event(&st, backers, vec![], &derivation, &serialization)?;
